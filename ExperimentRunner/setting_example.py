def Setup(Settings,DefaultModel):
    Settings["experiment_name"] = "OptimizersTest"
    Settings["dataset_name"] = "5556x_mark_res_299x299"

    Settings["number_of_images"] = 10
    Settings["models"][0]["epochs"] = 5
    Settings["models"][0]["unique_id"] = 'test-rmsprop'

    n = len(Settings["models"])
    Settings["models"].append(DefaultModel.copy())

    Settings["models"][n]["unique_id"] = 'test-adam'
    Settings["models"][n]["epochs"] = 5
    Settings["models"][n]["optimizer"] = 'adam'

    Settings["graph_histories"] = ['together'] #['all','together',[],[1,0],[0,0,0],[]]


    return Settings
