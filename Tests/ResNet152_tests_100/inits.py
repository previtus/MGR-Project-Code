...
def get_dataset():
    ...
    [x, y, x_val, y_val, osm, osm_val] = dataset.getDataLabels_split_with_osm(validation_split=model_settings["validation_split"])

    print len_(y), len_(y_val), len_(osm), len_(osm_val), len_(x), len_(x_val)

    return dataset, osm, osm_val, y, y_val, x, x_val
